...
describe("Catalog Routes", () => {
    describe("POST /products", () => {
        ...
        test("should response with an internal server error 500", async() => {
            const requestBody = mockRequest();
            jest
            .spyOn(catalogService, "createProduct")
    ...
    describe("PATCH /products/:id", () => {
        ...
        test("should response with an internal server error 500", async() => {
            ...
            jest
            .spyOn(catalogService, "updateProduct")
            .mockImplementationOnce(() => Promise.reject(new Error("unable to updaate product")));
            const response =  await request(app)
                .patch(`/products/${product.id}`)
                .send({...requestBody})
                .set("Accept","application/json")
            expect(response.status).toBe(500);
            expect(response.body).toEqual("unable to updaate product");
        });
    });
